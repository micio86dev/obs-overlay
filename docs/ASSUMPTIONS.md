...
21. **Quota is budgeted, not just retried.** The relay counts documented unit costs per call and degrades polling before the daily allowance runs out, because a 403 mid-stream is indistinguishable from the broadcast ending.
22. **A tombstone gets a prefixed event ID.** YouTube reuses the deleted message's ID for its tombstone, which the relay's own de-duplication would otherwise swallow; the moderation event is published as `tombstone:<id>` targeting the original.
23. **Poll tallies need no OAuth.** `snippet.pollDetails.metadata.options[].tally` is part of the public live-chat resource, so poll percentages are shown without owner credentials.
24. **Public identity is passed through, provider identity is not.** Display names and avatars are already public in YouTube's live chat and are needed to credit supporters; channel IDs are replaced with an opaque participant ID before broadcast.
25. **The page is transparent; only the chrome paints.** An OBS source placed under the browser source must show through the capture slots, so no full-bleed backdrop is drawn and the slots stay pure alpha.
26. **Capture slots are fixed 16:9 boxes.** They are sized from the height the chrome leaves free so a source can never overflow its frame at a different OBS canvas resolution.
