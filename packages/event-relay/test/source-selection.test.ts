...
test("createEventSource defaults to a healthy idle relay that emits no fabricated events", () => {
  const source = createEventSource({ mockIntervalMs: 8_000, pollIntervalMs: 10_000, dailyQuotaUnits: 10_000 });
...
test("createEventSource requires an explicit opt-in before enabling synthetic mock events", () => {
  assert.throws(
    () => createEventSource({ sourceName: "mock", mockIntervalMs: 8_000, pollIntervalMs: 10_000, dailyQuotaUnits: 10_000 }),
  ...
  assert.ok(createEventSource({
    ...
    pollIntervalMs: 10_000, dailyQuotaUnits: 10_000,
