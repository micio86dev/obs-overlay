  ...
  <section class="quiz-board" :class="state ? `phase-${state.phase}` : 'phase-loading'" aria-label="Python quiz board" aria-live="polite">
    <template v-if="state">
      ...
      <template v-else>
        ...
        <ol class="final-ranking"><li v-for="(player, index) in state.leaderboard.slice(0, 8)" :key="player.author"><span>#{{ index + 1 }}</span><img v-if="player.avatarUrl" class="avatar" :src="player.avatarUrl" alt="" loading="lazy" decoding="async" width="32" height="32" /><strong>{{ player.author }}</strong><b>{{ player.correctAnswers }}/{{ state.totalQuestions }}</b><em>{{ player.percentage }}%</em></li></ol>
