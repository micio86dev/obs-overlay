...
type LayoutName = "screen-webcam" | "screen-only" | "webcam-only" | "screen-camera";
const layouts: LayoutName[] = ["screen-webcam", "screen-only", "webcam-only", "screen-camera"];
...
<template>
  <main class="overlay" :class="[`layout-${layout}`, { demo: demoMode }]">
    ...
    <div class="status-branding">
      ...
      <div class="logo-frame" :role="demoMode ? 'img' : undefined" :aria-label="demoMode ? 'Logo placement' : undefined"><span v-if="demoMode">LOGO</span></div>
    </div>
    <section v-if="layout !== 'webcam-only'" class="slot screen-slot" :aria-hidden="demoMode ? undefined : 'true'" :aria-label="demoMode ? 'Screen capture placement' : undefined"><span v-if="demoMode">SCREEN CAPTURE</span></section>
    <section v-if="layout !== 'screen-only' && layout !== 'screen-camera'" class="slot webcam-slot" :aria-hidden="demoMode ? undefined : 'true'" :aria-label="demoMode ? 'Webcam placement' : undefined"><span v-if="demoMode">WEBCAM</span></section>
...
.slot, .logo-frame { display: grid; place-items: center; border: 1px solid transparent; background: transparent; }
...
.overlay.demo .slot { border-style: dashed; border-color: var(--color-accent-deep); color: var(--color-text-muted); background: var(--color-surface-transparent); }
.overlay.demo .logo-frame { border-color: var(--color-accent-deep); background: var(--color-surface-transparent); box-shadow: inset 0 0 0 1px var(--line-color); color: var(--color-text-muted); }
